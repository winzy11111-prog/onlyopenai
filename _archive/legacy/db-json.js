...
function load() {
    ...
    // Migration: add missing tables + fields
    let migrated = false;
    if (!db.chat_sessions) { db.chat_sessions = []; migrated = true; }
    if (!db._seq.chat_sessions) { db._seq.chat_sessions = 0; migrated = true; }
    // Phase 2: add thread_id to all existing sessions that don't have it
    db.chat_sessions.forEach(s => { if (s.thread_id === undefined) { s.thread_id = null; migrated = true; } });
    if (migrated) { save(db); console.log('✅ DB migrated: chat_sessions + thread_id'); }
    return db;
}
...
function query(sql, params = []) {
    ...
    // Login (bcrypt) — ดึง user by username เท่านั้น, server.js compare hash
    if (s.startsWith('SELECT id, username, display_name, role, plan, balance, project_id, password AS pw FROM users WHERE username=')) {
        const [username] = params;
        const rows = db.users
            .filter(u => u.username === username)
            .map(u => ({ id: u.id, username: u.username, display_name: u.display_name, role: u.role, plan: u.plan, balance: u.balance, project_id: u.project_id, pw: u.password }));
        return { rows };
    }
    ...
    // Update user (full edit — hash password only if changed)
    if (s.startsWith('UPDATE users SET display_name=')) {
        ...
        if (idx !== -1) {
            ...
            if (password && password !== '') {
                db.users[idx].password = bcrypt.hashSync(password, BCRYPT_ROUNDS);
            }
        }
        save(db);
        return { rows: [] };
    ...
    if (s.startsWith('INSERT INTO chat_sessions (user_id, title, skill_id, skill_name, skill_emoji, messages)')) {
        ...
        db.chat_sessions.unshift({
            ...
            thread_id:  null,   // Phase 2: OpenAI thread_id (set after first message)
            created_at: now,
            updated_at: now,
        });
        save(db);
    ...
    // Phase 2: Update session thread_id
    if (s.startsWith('UPDATE chat_sessions SET thread_id=')) {
        const [thread_id, id] = params;
        const idx = db.chat_sessions.findIndex(s => s.id === parseInt(id));
        if (idx !== -1) db.chat_sessions[idx].thread_id = thread_id;
        save(db);
        return { rows: [] };
    }
